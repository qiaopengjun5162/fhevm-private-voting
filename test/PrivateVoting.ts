...
describe("PrivateVoting", function () {
  ...
  it("records encrypted votes and reveals tallies after publication", async function () {
    ...
    const aliceVote = await fhevm.createEncryptedInput(privateVotingAddress, signers.alice.address).add8(1).encrypt();
    const bobVote = await fhevm.createEncryptedInput(privateVotingAddress, signers.bob.address).add8(0).encrypt();
    ...
    const tally0 = await fhevm.userDecryptEuint(FhevmType.euint8, encryptedTally0, privateVotingAddress, signers.alice);
    const tally1 = await fhevm.userDecryptEuint(FhevmType.euint8, encryptedTally1, privateVotingAddress, signers.alice);
    const tally2 = await fhevm.userDecryptEuint(FhevmType.euint8, encryptedTally2, privateVotingAddress, signers.alice);
